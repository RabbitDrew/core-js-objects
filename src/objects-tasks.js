...
function compareObjects(obj1, obj2) {
  const objEntr1 = Object.entries(obj1);
  const objEntr2 = Object.entries(obj2);
  return objEntr1.every((entry, i) => {
    return entry.every((el, j) => el === objEntr2[i][j]);
  });
}
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject).forEach((k) => {
    lettersObject[k].forEach((el) => {
      const subObj = {};
      subObj[k] = el;
      arrOfObj.push(subObj);
    });
  });

  arrOfObj.sort((a, b) => {
    const keyA = Object.keys(a).find((key) => typeof a[key] === 'number');
    const keyB = Object.keys(b).find((key) => typeof b[key] === 'number');
 ...
 */
function sellTickets(queue, count25 = 0, count50 = 0) {
  if (queue.length === 0) {
    return true;
  }
  const bill = queue[0];
  ...
  if (bill === 25) {
    return sellTickets(restQueue, count25 + 1, count50);
  }
  if (bill === 50) {
    if (count25 === 0) {
      return false;
    }
    return sellTickets(restQueue, count25 - 1, count50 + 1);
  }
  if (bill === 100) {
    ...
    if (count25 >= 3) {
      return sellTickets(restQueue, count25 - 3, count50);
    }
    return false;
  }
  return true;
...
function fromJSON(proto, json) {
  const prototype = Object.create(proto);
  const prop = JSON.parse(json);
  return Object.assign(prototype, prop);
}
...
class CssSelector {
  ...
  checkOrder(type) {
    ...
    // Проверка на уникальность element, id и pseudoElement
    if (type === 'element' && this.hasElement) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (type === 'id' && this.hasId) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (type === 'pseudoElement' && this.hasPseudoElement) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    ...
    if (
      this.order.length &&
      this.order[this.order.length - 1] > orderMap[type]
    ) {
      throw new Error(
    ...
    // Помечаем типы селекторов, чтобы не использовать их повторно
    if (type === 'element') this.hasElement = true;
    if (type === 'id') this.hasId = true;
    if (type === 'pseudoElement') this.hasPseudoElement = true;
  }
